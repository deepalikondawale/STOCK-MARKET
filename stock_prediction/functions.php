...
$datetime=new DateTime('today');
    for ($i=0; $i < 4 ; $i++) { 
      echo "<p>" .round($probabilities[$i]*100,2) . "% chance that  opening price of {$stock} will lie from $" . round($probable_range[$i],2) . " to $" . round($probable_range[$i+1],2) . "</p>";
      echo "<br/>";
    }
for($i=0;$i<4;$i++)
{
$x=(float)round($probable_range[$i],2);
$y=(float)round($probable_range[$i+1],2);
$j=(float)round($probabilities[$i],4);
$z='$'.$x.'- $'.$y;
$sql="INSERT INTO piechart1(id,x,y,range1,range2) VALUES('','$z','$j','$x','$y')";
...
global $connection;
    ...
    for ($i=0; $i < 4 ; $i++) { 
      echo "<p>" .round($probabilities[$i]*100,2) . "% chance that  closing price of {$stock} will lie from $" . round($probable_range[$i],2) . " to $" . round($probable_range[$i+1],2) . "</p>";
      echo "<br/>";
    }
...
$j=(float)round($probabilities[$i],4);
$z='$'.$x.'- $'.$y;
$sql="INSERT INTO piechart(id,x,y,range1,range2) VALUES('','$z','$j','$x','$y')";
mysqli_query($connection,$sql);
